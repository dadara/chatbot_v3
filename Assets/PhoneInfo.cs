...
public class PhoneInfo : MonoBehaviour {
	...
	Alice2 alice2;
	bool newHint = false;
	float hintChangeTime;
	bool firstHint = true;
	GameObject startscreen;
	public GameObject PhoneInfo2;
	...
	void Update () 
	{
		...
		} else
		{
			if(firstHint)
			{
				this.transform.GetComponent<UILabel>().text = "HINT:" + "\n" + "\n" + "This is your phone. It will display hints and additional information about the current topic of conversation. Maybe you can try showing her some of her pictures.";
				firstHint = false;
			}

			//Display new hint every 15 seconds
			if(!panel.GetComponent<UILogic>().inventoryIsActive && !panel.GetComponent<UILogic>().scanIsActive && !startscreen.activeSelf)
			{
				hintChangeTime += Time.deltaTime;
			}
			if(hintChangeTime>=15)
			{
				hintChangeTime=0;
				newHint = true;
			}
			...
			if(newHint)
			{
				...
				if(i == 0)
				{
					this.transform.GetComponent<UILabel>().text = "HINT:" + "\n" + "\n" + "It appears that Jane is reliving past experiences when you show her a picture of her past.";
				} else if(i == 1)
				{
					this.transform.GetComponent<UILabel>().text = "HINT:" + "\n" + "\n" + "Jane will sometimes go back to the past on her own.";
				} else if(i == 2)
				{
					this.transform.GetComponent<UILabel>().text = "HINT:" + "\n" + "\n" + "The quicker you complete minigames, the more points you will get.";
				} else if(i == 3)
				{
					this.transform.GetComponent<UILabel>().text = "HINT:" + "\n" + "\n" + "You can drag and drop an image from the inventory on the right onto Jane to show it to her.";
				}
		...
		//Copy Text to PhoneInfo2
		PhoneInfo2.GetComponent<UILabel>().text = this.transform.GetComponent<UILabel>().text;
	}
}
